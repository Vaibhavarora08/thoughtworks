# technogise Ratings Terraform

This repo has the terraform definitions to set up an AWS cluster for
the technogise Ratings app from my [blog](https://circleci.com/blog/deploy-a-clojure-web-application-to-aws-using-terraform/)
...
`technogise_key_pair`. Copy the `technogise_key_pair.pem` to your
`~/.ssh` directory and set the permissions like so: `chmod 400
~/.ssh/technogise_key_pair.pem`.
